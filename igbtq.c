...
int quantum_time(int i, int n, int qt[4]){
    int quantum,x;
    if(i<n/4){quantum=qt[0]; x=0;}
            else if(i<2*(n/4))   {quantum=qt[1]; x=1;}
            else if(i<3*(n/4))  {quantum=qt[2]; x=2;}
            else    {quantum=qt[3]; x=3;}
...
{
    int n, i;
    printf("==========  Improved Group Based Time Quantum CPU Scheduling Algorithm Simulator==========\n\n");
